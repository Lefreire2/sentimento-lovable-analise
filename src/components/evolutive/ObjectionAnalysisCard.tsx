...
export const ObjectionAnalysisCard = ({ data }: ObjectionAnalysisProps) => {
  const [correctedData, setCorrectedData] = useState(data?.objection_analysis);
  ...
  const analysis = correctedData || data.objection_analysis;
  
  // Validar dados de objeções universalmente
  const validation = validateUniversalAnalysis(analysis);

  const handleApplyCorrections = async (corrections: Record<string, any>) => {
    try {
      const newCorrectedData = correctConversionCalculations(analysis);
      setCorrectedData(newCorrectedData);
    } catch (error) {
      console.error('Erro ao aplicar correções em objeções:', error);
    }
  };
    ...
    <div className="space-y-4">
      ...
      <Tabs defaultValue="overview" className="space-y-6">
        ...
        <TabsContent value="sources" className="space-y-6">
          <LeadSourceAnalysisCard data={data} />
        </TabsContent>
